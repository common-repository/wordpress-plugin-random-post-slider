...
Text Domain: wordpress-plugin-random-post-slider
...
function gopiplus_admin_options() 
{
  ...
  <div class="form-wrap">
    ...
    <h2><?php _e('Random post slider', 'wordpress-plugin-random-post-slider'); ?></h2>
	<h3><?php _e('Plugin setting', 'wordpress-plugin-random-post-slider'); ?></h3>
	 <?php
	...
	if (isset($_POST['gopiplus_submit'])) 
	{
		...
		$gopiplus_displaydate = stripslashes(sanitize_text_field($_POST['gopiplus_displaydate']));
		$gopiplus_displaycategory = stripslashes(sanitize_text_field($_POST['gopiplus_displaycategory']));
		$gopiplus_displaycomment = stripslashes(sanitize_text_field($_POST['gopiplus_displaycomment']));
		$gopiplus_displayimage = stripslashes(sanitize_text_field($_POST['gopiplus_displayimage']));
		$gopiplus_displaytag = stripslashes(sanitize_text_field($_POST['gopiplus_displaytag']));
		$gopiplus_displaydesc = stripslashes(sanitize_text_field($_POST['gopiplus_displaydesc']));
		$gopiplus_displayreadmore = stripslashes(sanitize_text_field($_POST['gopiplus_displayreadmore']));
		$gopiplus_query_posts_showposts = stripslashes(sanitize_text_field($_POST['gopiplus_query_posts_showposts']));
		$gopiplus_query_posts_orderby = stripslashes(sanitize_text_field($_POST['gopiplus_query_posts_orderby']));
		$gopiplus_query_posts_order = stripslashes(sanitize_text_field($_POST['gopiplus_query_posts_order']));
		$gopiplus_query_posts_category = stripslashes(sanitize_text_field($_POST['gopiplus_query_posts_category']));
		$gopiplus_query_slider = stripslashes(sanitize_text_field($_POST['gopiplus_query_slider']));
		
		$gopiplus_displaydate = val_yn($gopiplus_displaydate);
		$gopiplus_displaycategory = val_yn($gopiplus_displaycategory);
		$gopiplus_displaycomment = val_yn($gopiplus_displaycomment);
		$gopiplus_displayimage = val_yn($gopiplus_displayimage);
		$gopiplus_displaytag = val_yn($gopiplus_displaytag);
		$gopiplus_displayreadmore = val_yn($gopiplus_displayreadmore);
		
		if(!is_numeric($gopiplus_displaydesc) || $gopiplus_displaydesc == 0) { $gopiplus_displaydesc = 300; }
		if(!is_numeric($gopiplus_query_posts_showposts) || $gopiplus_query_posts_showposts == 0) { $gopiplus_query_posts_showposts = 3; }
		
		if($gopiplus_query_posts_order != "ASC" && $gopiplus_query_posts_order != "DESC")
		{
			$gopiplus_query_posts_order = "ASC";
		}
		
		if($gopiplus_query_slider != "scrollLeft" && $gopiplus_query_slider != "scrollRight" && $gopiplus_query_slider != "scrollUp" && $gopiplus_query_slider != "scrollDown")
		{
			$gopiplus_query_slider = "scrollLeft";
		}
		
		if($gopiplus_query_posts_orderby != "ID" && $gopiplus_query_posts_orderby != "author" 
				&& $gopiplus_query_posts_orderby != "title" && $gopiplus_query_posts_orderby != "rand"
					 && $gopiplus_query_posts_orderby != "date" && $gopiplus_query_posts_orderby != "category" && $gopiplus_query_posts_orderby != "modified")
		{
			$gopiplus_query_posts_orderby = "ID";
		}
		...
		<div class="updated fade">
			<p><strong><?php _e('Details successfully updated.', 'wordpress-plugin-random-post-slider'); ?></strong></p>
		</div>
		<?php
	}
	 ?>
	 <form name="gopiplus_form" method="post" action="">
	  ...
	  <label for="tag-title"><?php _e('Display Date', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displaydate" id="gopiplus_displaydate" type="text" value="<?php echo $gopiplus_displaydate; ?>" />
      <p><?php _e('Enter YES or NO only ', 'wordpress-plugin-random-post-slider'); ?> (YES/NO)</p>
	  
	  <label for="tag-title"><?php _e('Display Category', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displaycategory" id="gopiplus_displaycategory" type="text" value="<?php echo $gopiplus_displaycategory; ?>" />
      <p><?php _e('Enter YES or NO only ', 'wordpress-plugin-random-post-slider'); ?> (YES/NO)</p>
	  
	  <label for="tag-title"><?php _e('Display Comment', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displaycomment" id="gopiplus_displaycomment" type="text" value="<?php echo $gopiplus_displaycomment; ?>" />
      <p><?php _e('Enter YES or NO only ', 'wordpress-plugin-random-post-slider'); ?> (YES/NO)</p>
	  
	  <label for="tag-title"><?php _e('Display Image', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displayimage" id="gopiplus_displayimage" type="text" value="<?php echo $gopiplus_displayimage; ?>" />
      <p><?php _e('Enter YES or NO only ', 'wordpress-plugin-random-post-slider'); ?> (YES/NO)</p>
	  
	  <label for="tag-title"><?php _e('Display Tag', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displaytag" id="gopiplus_displaytag" type="text" value="<?php echo $gopiplus_displaytag; ?>" />
      <p><?php _e('Enter YES or NO only ', 'wordpress-plugin-random-post-slider'); ?> (YES/NO)</p>
	  
	  <label for="tag-title"><?php _e('Display Readmore', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displayreadmore" id="gopiplus_displayreadmore" type="text" value="<?php echo $gopiplus_displayreadmore; ?>" />
      <p><?php _e('Enter YES or NO only ', 'wordpress-plugin-random-post-slider'); ?> (YES/NO)</p>
	  
	  <label for="tag-title"><?php _e('Display Content Length', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_displaydesc" id="gopiplus_displaydesc" type="text" value="<?php echo $gopiplus_displaydesc; ?>" />
      <p><?php _e('Enter only number ', 'wordpress-plugin-random-post-slider'); ?> (Ex: 300)</p>
	  
	  <label for="tag-title"><?php _e('Number of post to display', 'wordpress-plugin-random-post-slider'); ?></label>
      ...
      <p><?php _e('Enter only number ', 'wordpress-plugin-random-post-slider'); ?> (Ex: 3)</p>
	  
	  <label for="tag-title"><?php _e('Display post orderby', 'wordpress-plugin-random-post-slider'); ?></label>
      ...
      <p><?php _e('Enter any one from the list ', 'wordpress-plugin-random-post-slider'); ?> (ID / author / title / rand / date / category / modified)</p>
	  
	  <label for="tag-title"><?php _e('Display post order', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_query_posts_order" id="gopiplus_query_posts_order" type="text" value="<?php echo $gopiplus_query_posts_order; ?>" />
      <p><?php _e('Enter ASC or DESC only ', 'wordpress-plugin-random-post-slider'); ?> (ASC/DESC)</p>
	  
	  <label for="tag-title"><?php _e('Display post Categories', 'wordpress-plugin-random-post-slider'); ?></label>
      ...
      <p><?php _e('Category IDs, separated by commas', 'wordpress-plugin-random-post-slider'); ?> (Ex: 1, 2, 3)</p>
	  
	  <label for="tag-title"><?php _e('Slider Direction', 'wordpress-plugin-random-post-slider'); ?></label>
      <input name="gopiplus_query_slider" id="gopiplus_query_slider" type="text" value="<?php echo $gopiplus_query_slider; ?>" />
      <p><?php _e('Enter any one from the list', 'wordpress-plugin-random-post-slider'); ?> (scrollLeft / scrollRight / scrollUp / scrollDown)</p>
	  <input name="gopiplus_submit" id="gopiplus_submit" class="button-primary" value="<?php _e('Submit', 'wordpress-plugin-random-post-slider'); ?>" type="submit" />
	  <?php wp_nonce_field('gopiplus_form_setting'); ?>
	 </form>
	 </div>
  <br />
<p class="description">
	<?php _e('Check official website for more information', 'wordpress-plugin-random-post-slider'); ?>
	<a target="_blank" href="http://www.gopiplus.com/work/2011/05/28/wordpress-plugin-random-post-slider/"><?php _e('click here', 'wordpress-plugin-random-post-slider'); ?></a>
</p>
</div>
	<?php
}
...
{
	add_options_page( __('Random post slider', 'wordpress-plugin-random-post-slider'),  
				__('Random post slider', 'wordpress-plugin-random-post-slider'), 'manage_options', __FILE__, 'gopiplus_admin_options' );
}
...
function gopiplus_add_javascript_files() 
{
	if (!is_admin())
	{
		...
		wp_enqueue_script( 'jquery.cycle.all.latest', plugins_url().'/wordpress-plugin-random-post-slider/js/jquery.cycle.all.latest.js');
		wp_enqueue_style( 'wordpress-plugin-random-post-slider', plugins_url().'/wordpress-plugin-random-post-slider/wordpress-plugin-random-post-slider.css');
	}	
}

function gopiplus_textdomain() 
{
	  load_plugin_textdomain( 'wordpress-plugin-random-post-slider', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
